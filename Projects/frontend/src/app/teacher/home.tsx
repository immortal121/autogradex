...
import { FiHome, FiMoreHorizontal, FiSettings, FiUser, FiLogOut, FiFileText, FiEdit, FiTrash, FiArrowRight, FiShoppingCart, FiShoppingBag, FiType, FiPlusCircle, FiKey, FiUsers, FiBook, FiInfo, FiAirplay } from "react-icons/fi";
    ...
    <main className="flex bg-base-100 h-screen w-screen p-2 max-sm:p-0" onClick={() => {
      ...
      <div className={'print flex flex-col p-5 min-w-[275px] max-w-[15vw] h-full rounded-md ' + (!showMenu ? "max-sm:hidden " : "max-sm:fixed max-sm:w-full max-sm:h-full max-sm:max-w-none bg-base-100 max-sm:z-50 ")}>
        ...
        <div className='p-0 my-2 h-full w-full overflow-hidden hover:overflow-y-auto'>
          <Link href="/teacher"><label className={(!pathname.includes("/teacher") ? "btn-ghost " : "") + 'btn w-full justify-start normal-case'} onClick={() => { }}><FiHome /> Dashboard</label></Link>
          {/* <div className='p-0 ml-2 my-6 w-full overflow-hidden hover:overflow-y-auto'>
          <label>students</label>
          <Link href="/student"><label className={(!pathname.includes("/class") ? "btn-ghost " : "") + 'btn w-full justify-start normal-case'} onClick={() => { }}><FiAirplay /> Classes</label></Link>
          <Link href="/student"><label className={(!pathname.includes("/class") ? "btn-ghost " : "") + 'btn w-full justify-start normal-case'} onClick={() => { }}><FiAirplay /> Classes</label></Link>          
          </div> */}

          <SidebarItem
            label="Students"
            isActive={pathname.includes("/teacher/assignment")}
          >
            <SubSidebarItem href="/teacher/assignment" label="View All Assignments" />
            <SubSidebarItem href="/teacher/assignment/create" label="Create Assignment" />
          </SidebarItem>
        </div>
        <hr />
